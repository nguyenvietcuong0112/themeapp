...
import com.themes.diy.widgets.keyboard.controlcenter.feature_keyboard.model.Trie
...
class SuggestionBarView @JvmOverloads constructor(
    ...
    fun loadDictionaryAsync(locale: String) {
        thread {
            try {
                val dictName = "assets_keyboard/dicts/main_$locale.dict"
                val assetsList = context.assets.list("assets_keyboard/dicts") ?: emptyArray()
                val targetFile = if (assetsList.contains("main_$locale.dict")) dictName else "assets_keyboard/dicts/main_en-US.dict"
