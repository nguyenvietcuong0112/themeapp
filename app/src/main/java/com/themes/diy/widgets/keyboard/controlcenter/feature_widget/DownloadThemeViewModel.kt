...
class DownloadThemeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun deductCoins(amount: Int): Boolean {
        ...
        if (current >= amount) {
            ...
            com.themes.diy.widgets.keyboard.controlcenter.core.data.EventBus.getDefault().post(com.themes.diy.widgets.keyboard.controlcenter.core.data.CoinUpdatedEvent())
            return true
        }
        return false
    }
