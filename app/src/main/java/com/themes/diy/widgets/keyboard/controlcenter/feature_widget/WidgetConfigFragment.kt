...
import com.themes.diy.widgets.keyboard.controlcenter.feature_theme.creator.CreateThemeCategoryAdapter
...
class WidgetConfigFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        rvCategories = view.findViewById(R.id.categoryRecyclerView)
        rvWidgets = view.findViewById(R.id.recyclerView)
    ...
    private fun loadCategoriesAndWidgets() {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val jsonCategories = ServiceLocator.getThemeRepository(context).getCategories()
        ...
        for (theme in catData.themes) {
            ...
            widgetItems.add(
                WidgetTemplateItem(
                    ...
                    size = "4x2",
                    widgetType = "calendar",
                    previewUrl = "${ResourceConfig.ASSET_BASE_URL}/assets_theme/$path/widgets/today/bg_preview_medium.png",
                    themePath = path,
                    themeName = name
                )
            )
            ...
            widgetItems.add(
                WidgetTemplateItem(
                    ...
                    size = "2x2",
                    widgetType = "clock",
                    previewUrl = "${ResourceConfig.ASSET_BASE_URL}/assets_theme/$path/widgets/clocks/bg_preview_large.png",
                    themePath = path,
                    themeName = name
                )
            )
            ...
            widgetItems.add(
                WidgetTemplateItem(
                    ...
                    size = "2x2",
                    widgetType = "weather",
                    previewUrl = "${ResourceConfig.ASSET_BASE_URL}/assets_theme/$path/widgets/weather/bg_preview_large.png",
                    themePath = path,
                    themeName = name
                )
            )
            ...
            widgetItems.add(
                WidgetTemplateItem(
                    ...
                    size = "2x2",
                    widgetType = "date",
                    previewUrl = "${ResourceConfig.ASSET_BASE_URL}/assets_theme/$path/widgets/today/bg_preview_large.png",
                    themePath = path,
                    themeName = name
                )
            )
            ...
            widgetItems.add(
                WidgetTemplateItem(
                    ...
                    size = "2x2",
                    widgetType = "image",
                    previewUrl = "${ResourceConfig.ASSET_BASE_URL}/assets_theme/$path/widgets/image/bg_preview_large.png",
                    themePath = path,
                    themeName = name
                )
            )
    ...
    private fun onWidgetClicked(item: WidgetTemplateItem) {
        ...
        val dummyTheme = com.themes.diy.widgets.keyboard.controlcenter.feature_keyboard.data.entity.KeyboardTheme(
            id = uuidString,
            name = item.themeName,
            path = item.themePath,
            rawType = "widget_theme"
        )
        sheet.setParams(dummyTheme, item.widgetType, item.size, item.previewUrl, android.appwidget.AppWidgetManager.INVALID_APPWIDGET_ID)
        sheet.show(parentFragmentManager, "select_widget")
    }
